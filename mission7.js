...
app.get('/dump',function(req,res){
	...
	connection.query('SELECT * from sensors ORDER BY id DESC LIMIT 1440',function(err,rows,cols){
		...
		res.write('<html><head><title>Temperature</title></head><body>');		
		res.write('<p><h1>MISSION 7 - TEMPERATURE - AS 1016</h1></p>');
		res.write('<p>20121594 Park Jin Sang</p>');	
		...
		*/
		...
		res.write('<tr><th>No.</th><th>Time</th><th>Temperature</th></tr>');
		for(var i=0;i<rows.length;i++){
			var row=rows[i];
			res.write('<tr>');
